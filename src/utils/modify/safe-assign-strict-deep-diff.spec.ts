...
import { safeAssignStrictDeepDiff } from './safe-assign-strict-deep-diff';
...
describe('safeAssignStrictDeepDiff', () => {
	it('returns the "Same" symbol for equal object instances', () => {
		const arr: AnyArray = [];
		const obj = {};

		expect(safeAssignStrictDeepDiff(arr, arr)).to.be.equal(Same);
		expect(safeAssignStrictDeepDiff(obj, obj)).to.be.equal(Same);
	});
	...
	it('reassigns whole object tree without change and returns the "Same" symbol', () => {
		...
		const diff = safeAssignStrictDeepDiff(target, source);
	...
	it('reassigns parts of array using sparse array and returns sparse diff tree', () => {
		...
		const diff = <AnyArray>safeAssignStrictDeepDiff(target, source, true);
